...
comm_gl_ml <- detectCom(
  ...
  omega       = 1,
  seed        = 1,
  write_summary_csv = TRUE,   # writes com/size/span summary
  verbose     = TRUE
)
...
comm_clq_ml <- detectCom(
  ...
  k           = 4,
  m           = 2,
  write_summary_csv = TRUE,   # empty summary is written if no cliques found
  verbose     = TRUE
)
...
## 3) Per-layer enrichment restricted to a specific gene list; for example, common genes between layer E1 and layerE2
...
layerE1andE2_diff <- edges_diff_E1_E2$from[edges_diff_E1_E2$category == "common"] %>% unique()
head(layerE1andE2_diff)
...
gp_layer_custom <- gp_enrich_multinet(
  ...
  communities        = NULL,
  enrich_scope       = "layer",
  restrict_genes     = layerE1andE2_diff,   # <-- only these per layer
  min_query_size     = 3
)



## 4) Per-community enrichment restricted to a specific gene list like oxidative_stress_genes
...
gsea_comm_ox <- gsea_msigdbr_layer_pairs(
  ...
  results_dir          = getOption("mlnet.results_dir","omicsDNA_results"),
  run_name             = "gsea_pairs_byCommunity_oxidativeStress",
  top_n                = 8,
  format               = "pdf",
  show_in_rstudio      = TRUE,
  seed                 = 1,
...
gsea_layer_ox <- gsea_msigdbr_layer_pairs(
  ...
  enrich_scope         = "layer",
  restrict_genes       = oxidative_stress_genes,   # only these genes in ranking
  min_rank_genes       = 10,
