...
- [omicsDNA
  ...
  - [Function‑by‑function examples](#functionbyfunction-examples)
    ...
    - [21) `gsea_msigdbr_layer_pairs()` — **GSEA** (MSigDB, fgsea)
      across **consecutive layer
      pairs**](#21-gsea_msigdbr_layer_pairs--gsea-msigdb-fgsea-across-consecutive-layer-pairs)
    - [22) `go_enrichment_report()` — GO (clusterProfiler) **per module
      and per
      layer**](#22-go_enrichment_report--go-clusterprofiler-per-module-and-per-layer)
...
# Equivalent to multinet::glouvain_ml(net, gamma = 1, omega = 1)
set.seed(1)
comm_gl_ml <- detectCom(
  net,
  method      = "glouvain",
  supra_graph = FALSE,
  gamma       = 1,
  omega       = 1,
  seed        = 1,
  write_csv = TRUE,   # writes com/size/span summary
  verbose     = TRUE
)


# setting comm_gl_ml as default communty sets for downstream tasks
comm = comm_gl_ml

# make cids is integer 
comm$cid <- as.integer(comm$cid)

# Equivalent to multinet::infomap_ml()
comm_im_ml <- detectCom(
  net,
  method      = "infomap",
  supra_graph = FALSE,
  overlapping = FALSE,
...
# Equivalent to multinet::clique_percolation_ml()
comm_clq_ml <- detectCom(
  net,
  method      = "clique",
  supra_graph = FALSE,
  k           = 4,
  m           = 2,
  write_csv = TRUE,   # empty summary is written if no cliques found
  verbose     = TRUE
)



omm_im_supra <- detectCom(
  net,
  method      = "infomap",
  supra_graph = TRUE,
  edgeWeight  = "count",
...
head(comm_im_supra)
attr(comm_im_supra, "community_sizes")
attr(comm_im_supra, "layer_span")



# Clique-union components on the supra-graph
comm_clq_supra <- detectCom(
  net,
  method      = "clique",
  supra_graph = TRUE,
  k           = 4,            # clique size threshold on supra-graph
  edgeWeight  = "count",
  min.actors  = 20,
  min.layers  = 2,
  relabel_by_size = TRUE,
...
head(comm_clq_supra)
attr(comm_clq_supra, "community_sizes")
attr(comm_clq_supra, "layer_span")
```
...
### 20) `gp_enrich_multinet()` — g:Profiler enrichment **per module and per layer**

``` r

## 1) Per-layer enrichment
gp_layer <- gp_enrich_multinet(
  net                = net,
  organism           = "hsapiens",
  ...
  run_name           = paste0("gprof_byLayer_", format(Sys.time(), "%Y%m%d_%H%M%S")),
  ...
  ton_layout         = "fr",
  communities        = NULL,
  enrich_scope       = "layer",      # <-- per layer
  restrict_genes     = NULL,         # use all genes in each layer
  min_query_size     = 3
)
...
## 2) Per-community enrichment
gp_comm <- gp_enrich_multinet(
  net                = net,
  organism           = "hsapiens",
  sources            = c("GO:BP","REAC"),
  layer_order        = multinet::layers_ml(net),
  ...
  show_terms         = 10,
  results_dir        = getOption("mlnet.results_dir","omicsDNA_results"),
  run_name           = paste0("gprof_byCommunity_", format(Sys.time(), "%Y%m%d_%H%M%S")),
  per_layer_subdirs  = TRUE,
  format             = "png",
  width              = 9, height = 7, dpi = 300,
  show_in_rstudio    = TRUE,
  ...
  ton_min_overlap    = 1,
  ton_layout         = "fr",
  communities        = comm,         # actor/layer/cid (or com/community)
  enrich_scope       = "community",  # <-- per community
  restrict_genes     = NULL,         # all genes in each community
  min_query_size     = 3
)
...
## 3) Per-layer enrichment restricted to a specific gene list; for example, common genes between layerE1 and layerE2. Enrichment results will identify the molecular pathways in all the layers that are in the harmoney with E1 and E2 layers.

plot_layer_diff(
  net,
  L1 = "E1", L2 = "E2",
  layout = "fr", edge_alpha = 0.7, 
  save_edge_csv = TRUE, file = "diff_E1_vs_E2.png"
)

edges_diff_E1_E2 <- read_csv("omicsDNA_results/edges_diff_E1_vs_E2_2025-12-01_222225.csv")
head(edges_diff_E1_E2)
# A tibble: 6 × 5
#  from  to      category weight_L1 weight_L2
#  <chr> <chr>   <chr>        <dbl>     <dbl>
#1 BRCA2 RNF185  only_E1      0.643    NA    
#2 BRCA2 SF3B2   only_E1      0.676    NA    
#3 BRCA2 CFAP298 only_E1      0.586    NA    
#4 BRCA2 MCM10   common       0.613     0.687
#5 BRCA2 MKI67   only_E2     NA         0.807
#6 BRCA2 HELLS   only_E2     NA         0.690

E1andE2_common_genes <- edges_diff_E1_E2$from[edges_diff_E1_E2$category == "common"] %>% unique()
head(E1andE2_common_genes)
#[1] "BRCA2"   "BIRC5"   "AURKB"   "RNF185"  "CFAP298" "RAB22A" 

gp_layer_custom <- gp_enrich_multinet(
  net                = net,
  organism           = "hsapiens",
  sources            = c("GO:BP","REAC"),
  layer_order        = multinet::layers_ml(net),
  ...
  show_terms         = 10,
  results_dir        = getOption("mlnet.results_dir","omicsDNA_results"),
  run_name           = paste0("gprof_byLayer_oxidative_", format(Sys.time(), "%Y%m%d_%H%M%S")),
  per_layer_subdirs  = TRUE,
  format             = "png",
  width              = 9, height = 7, dpi = 300,
  show_in_rstudio    = TRUE,
  ...
  communities        = NULL,
  enrich_scope       = "layer",
  restrict_genes     = E1andE2_common_genes,  
  min_query_size     = 3
)
...
## 4) Per-layer enrichment restricted to a layer-specific gene list; for example, genes that are present in layer E1 but not E2, and see what other layers are in the molecular harmony with layer layer E1 but not E2.


layerE1_genes <- edges_diff_E1_E2$from[edges_diff_E1_E2$category == "only_E1"] %>% unique()
head(layerE1_genes)
#[1] "BRCA2"   "BIRC5"   "AURKB"   "RNF185"  "CFAP298" "RAB22A" 
...
## 5) Per-community enrichment restricted to a specific gene list like oxidative_stress_genes

pc_genes <- unique(subset(comm_annot, GeneType == "protein_coding")$actor)

oxidative_stress_genes <- pc_genes[1:100]   # your custom pathway gene set
head(oxidative_stress_genes)
#[1] "AAR2"  "ACBD6" "ACIN1" "ADNP"  "AGO1"  "ABCB5"

gp_comm_custom <- gp_enrich_multinet(
  net                = net,
  organism           = "hsapiens",
  sources            = c("GO:BP","REAC"),
  layer_order        = multinet::layers_ml(net),
  ...
  show_terms         = 10,
  results_dir        = getOption("mlnet.results_dir","omicsDNA_results"),
  run_name           = paste0("gprof_byCommunity_oxidative_", format(Sys.time(), "%Y%m%d_%H%M%S")),
  per_layer_subdirs  = TRUE,
  format             = "png",
  width              = 9, height = 7, dpi = 300,
  show_in_rstudio    = TRUE,
  ...
  ton_min_overlap    = 1,
  ton_layout         = "fr",
  communities        = comm,                    # actor / layer / cid/com
  enrich_scope       = "community",
  restrict_genes     = oxidative_stress_genes,  # <-- only these per community
  min_query_size     = 3
)
```

### 21) `gsea_msigdbr_layer_pairs()` — **GSEA** (MSigDB, fgsea) across **consecutive layer pairs**

``` r

## 1) Community‑scope GSEA (all genes per community per pair)
gsea_comm <- gsea_msigdbr_layer_pairs(
  net                  = net,
  layer_order          = multinet::layers_ml(net),
  communities          = comm,                      # actor / layer / cid
  enrich_scope         = "community",              # per pair × community
  ...
  results_dir          = getOption("mlnet.results_dir","omicsDNA_results"),
  run_name             = "gsea_pairs_byCommunity_allGenes",
  top_n                = 8,
  format               = "png",
  show_in_rstudio      = TRUE,
  seed                 = 1,
...
## 2) Layer‑scope GSEA (all genes per layer pair)
gsea_layer <- gsea_msigdbr_layer_pairs(
  net                  = net,
  layer_order          = multinet::layers_ml(net),   # traversal order
  scores_by_layer      = NULL,                       # use vertex attributes
  cor_attr             = c("cor","r","rho","correlation"),
  padj_attr            = c("padj","FDR","q","adj_p","p_adj","p.adj","p_fdr"),
  enrich_scope         = "layer",                    # one GSEA per layer pair
  ...
  minSize              = 15,
  maxSize              = 500,
  nperm                = NULL,                       # fgseaMultilevel()

  results_dir          = getOption("mlnet.results_dir","omicsDNA_results"),
  run_name             = "gsea_pairs_layer_allGenes",
  top_n                = 10,
  format               = "png",
  show_in_rstudio      = TRUE,
  seed                 = 1,
...
## 3) Community‑scope GSEA restricted to a custom gene list
plot_layer_diff(net, L1 = "E1", L2 = "M1", save_edge_csv = TRUE)
edges_diff_E1_M1 <- read_csv("omicsDNA_results/edges_diff_E1_vs_E2_2025-12-01.csv")

E1_M1 <- edges_diff_E1_M1$from[edges_diff_E1_E2$category == "common"] %>% unique()

gsea_comm_ox <- gsea_msigdbr_layer_pairs(
  net                  = net,
  layer_order          = multinet::layers_ml(net),
  communities          = comm,
  enrich_scope         = "community",
  restrict_genes       = E1_M1,  
  min_rank_genes       = 10,

  msigdb_species       = "Homo sapiens",
  msigdb_collections   = c("H","C2"),
  msigdb_subcategories = "CP:KEGG",
  ...
  results_dir          = getOption("mlnet.results_dir","omicsDNA_results"),
  run_name             = "gsea_pairs_byCommunity_E1_M1_CommonGenes",
  top_n                = 8,
  format               = "pdf",
  show_in_rstudio      = TRUE,
  seed                 = 1,
...
4) Layer‑scope GSEA restricted to a custom gene list
gsea_layer_ox <- gsea_msigdbr_layer_pairs(
  net                  = net,
  layer_order          = multinet::layers_ml(net),
  ...
  results_dir          = getOption("mlnet.results_dir","omicsDNA_results"),
  run_name             = "gsea_pairs_layer_oxidativeStress",
  top_n                = 12,
  format               = "pdf",
  show_in_rstudio      = TRUE,
  seed                 = 1,
...
### 22) `go_enrichment_report()` — GO (clusterProfiler) **per module and per layer**

``` r

## 1) Per-community (all genes per community)
go_comm <- go_enrichment_report(
  net                      = net,
  communities              = comm,   # detectCom() output
  OrgDb                    = org.Hs.eg.db,
  id_from                  = "SYMBOL",
  ont                      = "BP",
  enrich_scope             = "community",
...
## 2) Per-layer (all genes per layer)
...
## 3) Per-community enrichment restricted to a specific gene list

go_comm_ox <- go_enrichment_report(
  net                      = net,
  communities              = comm,
  restrict_genes           = c("RHCG","SPRR1A","SPRR2A","SPRR3","KRT13","KRT4","CRNN"),
  OrgDb                    = org.Hs.eg.db,
  id_from                  = "SYMBOL",
  ont                      = "BP",
  enrich_scope             = "community",
  layer_order              = multinet::layers_ml(net),
  universe_mode            = "union",
  pAdjustMethod            = "none",     # or "fdr", "bonferroni", "BH"
  pvalueCutoff             = 0.05,
  include_barplot_multinet = FALSE,
  cnet_showCategory        = 8,
  cnet_left_margin_pt      = 100,
  ...
  save_pdf                 = TRUE,
  one_pdf                  = FALSE,
  file_prefix              = "customGenes",
  out_dir                  = getOption("mlnet.results_dir","omicsDNA_results")
)



## 4) Per-layer enrichment restricted to a specific geneset like oxidative_stress_genes
