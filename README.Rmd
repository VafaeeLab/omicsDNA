...
> Tip: set a default results directory so all artefacts land in one place.
```r
...
Below, each function has **one example with its most informative arguments** (as in the package script). Replace object names (`expression_mat`, `metaData`, …) with your own data.
...
For a comprehensive, detailed analysis, refer to the following vignette on human foetal kidney single-cell RNA-seq data:
...
Build a second graph so that two multilayer graphs can be compared with the next function.
```r
...
**compare_multinets() compares the common and differing edges between the layers of two multilayer graphs. It can be used in downstream enrichment analysis to compare two biological contexts across the same time points or layers.**
...
Most functions save outputs under `getOption("mlnet.results_dir","omicsDNA_results")` with timestamped names; many return a `file` / `files` attribute pointing to what was written. Set a seed before any stochastic step (resampling, layouts, community detection):
